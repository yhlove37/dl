					...
					<table cellspacing="0" cellpadding="1" rules="all"
						...
						<c:forEach  var="user1" items="${users}">
							<tr onmouseover="this.style.backgroundColor = 'white'"
								...
								<td align="center" style="HEIGHT: 22px">
									<c:choose>
										<c:when test="${user1.state==1}">
											<button class="but-primary" onclick="window.location.href='${pageContext.request.contextPath}/user/activebyuid?uid=${user1.uid}&&type=0'">禁用用户</button>
										</c:when>
										<c:when test="${user1.state==3}">
											<a>管理员</a>
										</c:when>
									<c:otherwise>
										<button class="but-primary" onclick="window.location.href='${pageContext.request.contextPath}/user/activebyuid?uid=${user1.uid}&&type=1'">激活用户</button>
								...
								<td align="center" style="HEIGHT: 22px">
									<a href="${pageContext.request.contextPath}/user/deletebyid?uid=${user1.uid}">删除当前用户</a>
								</td>
